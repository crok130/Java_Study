...
<body>
	...
	<!-- 
	 ...
	 -->
	 <br/>
	 medium : <f:formatDate value="${now}" /> <br/>
	 full : <f:formatDate value="${now}" type="date" dateStyle="full" /><br/>
	 ...
	 <%= response.getLocale() %>
	 앤화 : <f:formatNumber type="currency" value="${param.price}"/> <br/>
	 <f:setLocale value="en_US"/>
	 달러 : <f:formatNumber type="currency" value="${param.price}"/> <br/>
	 
	 <f:setLocale value="th_TH_#Thai"/>
	 th_TH_#Thai : <f:formatNumber type= "currency" value="${param.price}"/> <br/>
	 <h3>pattern</h3>
	 <!-- ㄹ + 한자 + 3 -->
	 ...
	 <h4>이름 : ${name} - ${sessionScope.name}</h4>
	 <h4>주소 : ${addr}</h4>
	 <h4>전화번호 : ${phone}</h4>
	 <hr/>
	 <h3> 국가별 정보 </h3>
	 <!-- 
	 	등록된 국가 정보
	 	Locale class에 등록된 사용가능한 국가 코드 정보 
 	-->
	 <c:forEach var="loc" items="<%=java.util.Locale.availableLocales().iterator() %>">
	 	display-country - ${loc.getDisplayCountry()} <br/>
