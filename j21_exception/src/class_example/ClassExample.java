...
public class ClassExample {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Person p0 = clazz.getConstructor().newInstance(); // 기본 생성자
			// new Person(String)
			Person p1 = clazz.getConstructor(String.class).newInstance("김동하");
			p1.setAge(65);
			System.out.println(p1);
			
			Person p2 = clazz.getConstructor(String.class, int.class).newInstance("최수빈",67);
			...
			// newInstance로 생성된 객체의 method 또는 생성자에서 예외가 발생되었을 떄
			// 그 오류를 전달 - 생성자내부에서 발생된 NUllPointException
		} catch (NoSuchMethodException e) {
			// 오버로딩된 생성자를 찾지 못함.
			// 동일한 개수와 매개변수 타입을 가진 생성자를 찾지 못함.
		} catch (SecurityException e) {
			
			// 접근 권환이 없을 때 - 파일이 접근할 수 없는 디렉토리에 존재할 때
		}
